...
# ///
"""
wotch_downloader.py — Download all pages from https://www.thewotch.com/
and compress them into a zip archive.

Usage:
    pipx run wotch_downloader.py [options]

Options:
    --output DIR     Directory to save images (default: wotch_pages)
    --zip FILE       Output zip filename (default: wotch.zip)
...
Notes:
    ...
    - The comic slug is taken from the ?comic= query parameter.
...
try:
    ...
    print("  pipx run wotch_downloader.py")
...
START_URL = "https://www.thewotch.com/?comic=enter-the-wotch"
BASE_URL = "https://www.thewotch.com"
...
# WordPress comic plugin selectors, then generic fallbacks
IMAGE_SELECTORS = [
    "#comic img",
    "#comic-image",
    "#comicimage",
    ...
    for selector in IMAGE_SELECTORS:
        ...
        if tag:
            ...
            if img and img.get("src"):
                ...
                if _looks_like_comic_image(src):
                    return urljoin(current_url, src)

    # Last-resort: any wp-content/uploads image that looks like a dated upload
...
def _looks_like_comic_image(src: str) -> bool:
    """Return True if the URL looks like an uploaded comic image (not a theme asset)."""
    return "wp-content/uploads" in src
...
def find_next_url(soup: BeautifulSoup, current_url: str) -> str | None:
    ...
    Navigation links are plain <a> tags with text like "Next ›"; the Next link
    has no href (or href="#") on the last page.
    """
    current_path = urlparse(current_url)

    for a in soup.find_all("a"):
        text = a.get_text(strip=True).lower()
        if "next" in text and "›" in a.get_text():
            href = a.get("href", "").strip()
            if not href or href == "#":
                return None
            abs_url = urljoin(current_url, href)
            # Guard against circular navigation
            if urlparse(abs_url).query == current_path.query:
                return None
            return abs_url
...
def slug_from_url(url: str) -> str:
    """Extract the comic slug from the ?comic= query parameter."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download The Wotch webcomic and compress to a zip.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--output", default="wotch_pages", metavar="DIR",
                        help="Directory for downloaded images (default: wotch_pages)")
    parser.add_argument("--zip", default="wotch.zip", metavar="FILE",
                        help="Output zip filename (default: wotch.zip)")
    ...
    parser.add_argument("--debug-nav", action="store_true",
                        help="Dump all <a> tags and comic images from the first page and exit")
...
def _debug_nav(start_url: str, timeout: int) -> None:
    ...
    print(f"\nImages with wp-content/uploads in src:")
    for img in soup.find_all("img"):
        src = img.get("src", "")
...
def main() -> None:
    ...
    print("=" * 60)
    print("The Wotch — Comic Downloader")
    print("=" * 60)
    if args.limit:
